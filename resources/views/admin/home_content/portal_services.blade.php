...
<body>
    ...
    $(document).ready(function() {
        ...
        var table = $('#content-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('portal-services.index') }}",
            columns: [
        ...
        $('#addContentForm').on('submit', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('portal-services.store') }}",
        ...
        $(document).on('click', '.edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('portal-services.index') }}" + '/' + id + '/edit', function(data) {
                $('#editId').val(data.id);
        ...
        $('#editContentForm').on('submit', function(e) {
            ...
            $.ajax({
                type: "PUT",
                url: "{{ route('portal-services.update', '') }}/" + id,
        ...
        $('#confirmDelete').on('click', function() {
            ...
            $.ajax({
                type: "DELETE",
                url: "{{ route('portal-services.destroy', '') }}/" + id,
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function(response) {
