...
<body>
                ...
                <div class="modal-body">
                    <form id="addContentForm">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <div id="videoDescriptionEditor"></div>
                            <input type="hidden" id="videoDescription" name="videoDescription">
                        </div>
                ...
                <div class="modal-body">
                    <form id="editContentForm">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" id="editVideoTitle" name="videoTitle" required>
                        </div>
                        <div class="form-group mb-3">
                            <label for="editVideoDescription">Video Description</label>
                            <div id="editVideoDescriptionEditor"></div>
                            <input type="hidden" id="editVideoDescription" name="videoDescription">
                        </div>
    ...
    $(document).ready(function() {
        ...
        var videoDescriptionQuill = new Quill('#videoDescriptionEditor', { theme: 'snow' });
        var editVideoDescriptionQuill = new Quill('#editVideoDescriptionEditor', { theme: 'snow' });

        var table = $('#content-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('covid-19.index') }}",
            columns: [
            ...
            return data;
                ...
                {data: 'link', name: 'link'},
                {data: 'videoTitle', name: 'videoTitle'},
                {
        ...
        $('#addContentForm').on('submit', function(e) {
            ...
            $('#videoDescription').val(videoDescriptionQuill.root.innerHTML);
            $.ajax({
                type: "POST",
                url: "{{ route('covid-19.store') }}",
                ...
                success: function(response) {
                    ...
                    videoDescriptionQuill.root.innerHTML = '';
                    table.ajax.reload();
        ...
        $(document).on('click', '.edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('covid-19.index') }}" + '/' + id + '/edit', function(data) {
                $('#editId').val(data.id);
                $('#editTitle').val(data.title);
                editTitleQuill.root.innerHTML = data.title;
                editQuill.root.innerHTML = data.content;
                $('#editLink').val(data.link);
                $('#editVideoTitle').val(data.videoTitle);
                editVideoDescriptionQuill.root.innerHTML = data.videoDescription;
        ...
        $('#editContentForm').on('submit', function(e) {
            ...
            $('#editVideoDescription').val(editVideoDescriptionQuill.root.innerHTML);
            var id = $('#editId').val();
            $.ajax({
                type: "PUT",
                url: "{{ route('covid-19.update', '') }}/" + id,
