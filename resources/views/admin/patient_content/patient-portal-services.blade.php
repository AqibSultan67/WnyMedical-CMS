...
<head>
    ...
    <title>Patient Portal Services</title>
...
<body>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var addTitleQuill = new Quill('#addTitleEditor', {
                theme: 'snow',
                modules: {
                    toolbar: toolbarOptions
                }
            ...
            var editTitleQuill = new Quill('#editTitleEditor', {
                theme: 'snow',
                modules: {
                    toolbar: toolbarOptions
                }
            ...
            var table = $('#content-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('patient-portal-services.index') }}",
                columns: [
            ...
            $('#addContentForm').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('patient-portal-services.store') }}",
    ...
    $('.modal-backdrop').remove(); 
            ...
            $(document).on('click', '.editContent', function() {
                var id = $(this).data('id');
                $.get("{{ route('patient-portal-services.index') }}" + '/' + id + '/edit', function(data) {
                    $('#editId').val(data.id);
            ...
            $('#editContentForm').on('submit', function(e) {
                ...
                $.ajax({
                    type: "PUT",
                    url: "{{ route('patient-portal-services.update', '') }}/" + id,
            ...
            $('#confirmDelete').on('click', function() {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('patient-portal-services.destroy', '') }}/" + id,
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
