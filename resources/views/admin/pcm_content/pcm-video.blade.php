...
<body>
    ...
    $(document).ready(function() {
        ...
        var table = $('#content-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('pcm-video-link.index') }}",
            columns: [
        ...
        $('#addContentForm').on('submit', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('pcm-video-link.store') }}",
                data: $(this).serialize(),
        ...
        $(document).on('click', '.edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('pcm-video-link.index') }}" + '/' + id + '/edit', function(data) {
                $('#editId').val(data.id);
        ...
        $('#editContentForm').on('submit', function(e) {
            ...
            $.ajax({
                type: "PUT",
                url: "{{ route('pcm-video-link.update', '') }}/" + id,
                data: $(this).serialize(),
                success: function(response) {
        ...
        $('#confirmDelete').on('click', function() {
            ...
            $.ajax({
                type: "DELETE",
                url: "{{ route('pcm-video-link.destroy', '') }}/" + id,
                data: {
                    _token: '{{ csrf_token() }}'},
                success: function(response) {
