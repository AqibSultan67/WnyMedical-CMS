...
<head>
    ...
    <title>Services Title Images</title>
...
<script type="text/javascript">
    ...
    $(function () {
        var table = $('#slider-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('services-title-image.index') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'image', name: 'image', render: function(data) {
                    return `<img src="/storage/services/${data}" style="width:50%;" />`;
                }},
        ...
        $('#addSliderForm').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('services-title-image.store') }}",
                data: formData,
                contentType: false,
                processData: false,
    ...
    var editUrl = "/admin/dashboard/services-title-image/" + sliderId + "/edit";
    console.log("Fetching data from: " + editUrl);

    $.get(editUrl, function(data) {
        $('#sliderId').val(data.id);
        $('#editImagePreview').attr('src', '/storage/services/' + data.image).show();
        ...
        $('#editSliderForm').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('services-title-image.update', '') }}/" + sliderId,
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
        ...
        $('#confirmDelete').click(function() {
            ...
            $.ajax({
                type: 'DELETE',
                url: "{{ route('services-title-image.destroy', '') }}/" + sliderId,
                data: {
                _token: '{{ csrf_token() }}'},
                success: function(response) {
