...
<script type="text/javascript">
    ...
    $(function () {
        var table = $('#slider-table').DataTable({
            ...
            columns: [
                ...
                {data: 'image', name: 'image', render: function(data) {
                    return `<img src="/storage/slider-images/${data} "style="width: 100%; height:50%;" />`;
                }},
        ...
        $(document).on('click', '.edit', function() {
            ...
            var editUrl = "{{ route('admin.sliders.edit', '') }}/" + sliderId;
            console.log("Fetching data from: " + editUrl);
            $.get(editUrl, function(data) {
                $('#sliderId').val(data.id);
                $('#editImagePreview').attr('src', '/storage/slider-images/' + data.image).show();
                $('#editSliderModal').modal('show');
            }).fail(function() {
                alert('Could not retrieve slider data');
            });
        });
        ...
        $('#editSliderForm').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.sliders.update', '') }}/" + sliderId,
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
        ...
        $('#confirmDelete').click(function() {
            ...
            $.ajax({
                type: 'DELETE',
                url: "{{ route('admin.sliders.destroy', '') }}/" + sliderId,
                data: {
                _token: '{{ csrf_token() }}'},
                success: function(response) {
